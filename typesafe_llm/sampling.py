...
from typesafe_llm.typestate.protocols import file_api_automaton
from typesafe_llm.typestate.parser import APICallParser
from typesafe_llm.typestate.incremental_parser import (
    incremental_typestate_parse,
    custom_end_initial_state_typestate,
    TypestateParserState
)
import re
...
def sample_constrained(
    ...
    elif constrain_language == "typestate":
        if typestate_automaton is None:
            typestate_automaton = file_api_automaton()
        parser = APICallParser(typestate_automaton)
        # Use the incremental typestate parser
        incremental_parse = incremental_typestate_parse
        initial_incremental_parse = partial(
            incremental_typestate_parse,
            custom_end_initial_state_typestate(constrain_until, typestate_automaton),
        )
    else:
        ...
        try:
            ...
            for _ in range(max_tokens):
                ...
                if states is None and constrain:
                    if constrain_language == "typestate":
                ...
                if constrain:
                    if constrain_language == "typestate":
                        checking_str = f"(typestate: {states.automaton.state}) ... "
                    else:
                ...
                if constrain:
                    if constrain_language == "typestate":
                        eos_ok = states.accept if states else False
                    else:
                    ...
                    if not found:
                        if constrain_language == "typestate":
                            # For typestate, we don't use the trie-based approach
                            # The incremental parser handles the constraints
                            pass
                        else:
                ...
                if next_token_index.item() == tokenizer.eos_token_id:
                    ...
                    if constrain_language == "typestate":
                        ...
                        eos_ok = any(s.accept for s in next_states) if 'next_states' in locals() else False
                    
                    if not constrain or eos_ok:
                        delete(checking_str, _pflush)
